  ...
  const fetchEntries = useCallback(async () => {
    try {
      ...
      const res = await axios.get(
        `${API_URL}/entry/admin/entries?page=${currentPage}&limit=${itemsPerPage}`,
        getHeaders(),{}
      );
      if (res.data.success) {
...
<div className="flex items-center gap-2 text-sm text-gray-600">
  <span className="font-medium text-gray-700">
    ...
    <span className="text-indigo-600">
      {data.length === 0 ? 0 : (pagination.currentPage - 1) * pagination.limit + 1}
    </span>
    {"–"}
    <span className="text-indigo-600">
    ...
    <div className="flex flex-wrap gap-3 items-center">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-600">
        <label htmlFor="entriesPerPage" className="font-medium text-gray-700">
          Show:
        </label>
        <select
          id="entriesPerPage"
