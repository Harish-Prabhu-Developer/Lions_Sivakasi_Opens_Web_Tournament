...
function Navbar() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("storage", (e) => {
      if (e.key === "bulkapp_token" || e.key === "user_bulkapp_Data") {
        updateLoginState();
      }
    });
    ...
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 border-b border-white/10 backdrop-blur-lg ${
        scrolled
          ...
          : "bg-linear-to-r from-[#0a192f]/90 to-[#0f223f]/90"
      }`}
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 md:px-10 h-[70px]">
        ...
        <ul className="hidden md:flex items-center space-x-10">
          {navLinks.map((link) => {
            ...
            return (
              <li key={link.name}>
                <Link
                  to={link.path}
                  className={`relative transition-all duration-300 after:absolute after:left-0 after:-bottom-1 after:h-0.5 after:bg-cyan-400 after:transition-all after:duration-300 ${
          ...
          {!isLoggedIn && (
            <Link
              to="/auth?activeTab=register"
              className="hidden md:inline-flex items-center justify-center w-36 h-10 rounded-full bg-linear-to-r from-cyan-600 to-cyan-400 tracking-wide shadow-md hover:from-cyan-500 hover:to-cyan-300 active:scale-95 transition-all duration-300"
            >
          ...
          <ul className="flex flex-col items-center space-y-4 text-white text-lg">
            ...
            {!isLoggedIn && (
              <Link
                to="/auth?activeTab=register"
                onClick={() => setIsOpen(false)}
                className="w-40 h-11 rounded-full bg-linear-to-r from-cyan-600 to-cyan-400 flex items-center justify-center shadow-md hover:from-cyan-500 hover:to-cyan-300 active:scale-95 transition-all duration-300"
              >
